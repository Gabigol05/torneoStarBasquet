...
export function useFemeninoStats(enabled = true, temporadaId = null) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event:'*', schema:'public', table:'equipos_femenino'        }, refreshDebounced)
      .on('postgres_changes', { event:'*', schema:'public', table:'jugadoras_femenino'      }, refreshDebounced)
      .on('postgres_changes', { event:'*', schema:'public', table:'estadisticas_femenino'  }, refreshDebounced)
      .on('postgres_changes', { event:'*', schema:'public', table:'partidos_femenino'       }, refreshDebounced)
      .on('postgres_changes', { event:'*', schema:'public', table:'stats_partido_femenino'  }, refreshDebounced)
      .on('postgres_changes', { event:'*', schema:'public', table:'fechas_femenino'         }, refreshDebounced)
      .subscribe((status) => {
