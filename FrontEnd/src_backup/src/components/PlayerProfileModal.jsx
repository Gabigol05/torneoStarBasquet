...
function TiroRow({ label, conv, fall, pct, color, promConv }) {
  const total = conv + fall;
  const pctReal = total > 0 ? Math.round((conv / total) * 100) : pct;
  return (
...
export function PlayerProfileModal({ player, isOpen, onClose, statsPorPartido, partidos, fechas }) {
  ...
  const hayStats = pj > 0;
  const hayTiros = (scTotal + dcTotal + tcTotal) > 0;

  // Historial por fecha para el gráfico
      ...
      <div className="pp-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="pp-card">
          ...
          <div className="pp-avatar-container">
            <div className="pp-avatar-initials" style={{ borderColor: player.color ?? 'var(--fem2)', color: player.color ?? 'var(--fem2)' }}>
          ...
          {hayStats ? (
            <>
              ...
              <div style={{ display:'flex', gap:10, marginBottom:20, flexWrap:'wrap' }}>
                ...
                <StatCard val={mejorPts} lbl="Mejor partido"    color="#F0B429" sub="PTS"/>
                    ...
                    {[
                      ...
                      <div key={t.lbl} style={{ flex:1, minWidth:80, textAlign:'center', background:'#141C2A', borderRadius:8, padding:'8px 6px' }}>
                        <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:22, color:t.color, lineHeight:1 }}>
                          {t.conv}<span style={{ color:'#4A566E', fontSize:14 }}>/{t.conv+t.fall}</span>
                        </div>
                ...
                <div>
                  ...
                  <div style={{ overflowX:'auto' }}>
                    <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                      <thead>
                        <tr>
                          {['Fecha','PTS','REB','AST','TL','2P','3P'].map(h => (
                            <th key={h} style={{ color:'#6B7A99', padding:'6px 8px', textAlign:'center', fontSize:10, borderBottom:'1px solid #1C2535', whiteSpace:'nowrap' }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {historialGrafico.map((r, i) => (
                          <tr key={i} style={{ background: i%2===0 ? 'transparent':'rgba(255,255,255,.02)' }}>
                            ...
                            <td style={{ color:'#22D07A', padding:'7px 8px', textAlign:'center', fontSize:11 }}>
                              {r.sc}/{r.sc+r.sf}
                            </td>
                            <td style={{ color:'#F0B429', padding:'7px 8px', textAlign:'center', fontSize:11 }}>
                              {r.dc}/{r.dc+r.df}
                            </td>
                            <td style={{ color:'#60A5FA', padding:'7px 8px', textAlign:'center', fontSize:11 }}>
                              {r.tc}/{r.tc+r.tf}
                            </td>
                          </tr>
                        ))}
